...
public interface TradeService {
	...
	default Trade dtoToEntity(TradeDTO dto) {
		...
		Product product = Product.builder().pnum(90L).build();
//		Product product = Product.builder().pNum()
				
		
		Trade trade = Trade.builder().tNum(dto.getTNum()).bWallet(dto.getBWallet())
	...
	public static TradeDTO entityToDTO(Product product, Trade trade) {
		TradeDTO tradeDTO = TradeDTO.builder()
				...
				.pNum(product).txid(trade.getTxid()).total(trade.getTotal())
